...
/**
 * Renders a stylized alphanumeric glitch animation on a canvas backdrop.
 * 
 * Used for thematic branding in high-impact UI areas like the login screen.
 */
const LetterGlitch = ({
  ...
  useEffect(() => {
    ...
    const parseColor = (col: string): { r: number; g: number; b: number } | null => {
      const rgbMatch = /^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/.exec(col);
      if (rgbMatch) {
        return {
      ...
      const hex = col.replace(shorthandRegex, (_m, r, g, b) => r + r + g + g + b + b);
    ...
    const calculateGrid = (w: number, h: number) => {
      ...
      const rs = Math.ceil(h / charHeight);
      return { columns: cols, rows: rs };
    };

    const initializeLetters = (cols: number, rs: number) => {
      grid.current = { columns: cols, rows: rs };
      const totalLetters = cols * rs;
  ...
  }, [glitchSpeed, smooth, characters, glitchColors, outerVignette, centerVignette]);
