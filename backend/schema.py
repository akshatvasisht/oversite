...
class Session(Base):
    """
    Represents an active or completed assessment session for a candidate.
    """
...
class File(Base):
    """
    Maintains the state of individual files within a session's workspace.
    """
    __tablename__ = 'files'
...
class Event(Base):
    """
    Audit log of high-level candidate actions, such as executions or panel focus.
    """
    ...
    def to_dict(self):
        """
        Serializes the event for JSON transmission.

        Returns:
            A dictionary representation of the event audit.
        """
        return {
...
class AIInteraction(Base):
    """
    Logs chat-based turns between the candidate and the AI assistant.
    """
    ...
    def to_dict(self):
        """
        Serializes the interaction for analytics consumption.

        Returns:
            A dictionary containing prompt and response metadata.
        """
        return {
...
class SessionScore(Base):
    """
    Stores the final evaluation artifacts generated by the behavioral scoring pipeline.
    """
