...
# Applied per-item based on the *total* number of units in the cart.
# Format: (minimum_quantity, discount_per_unit)
# ---------------------------------------------------------------------------
...
class DiscountEngine:
    ...
    Discounts must be applied in the following order:
      ...
      1. Quantity discounts  — a flat amount off per unit, tiered by cart size.
                               Reduces the effective price of every item.
    ...
    def apply(self, items: list[CartItem], coupon: str | None = None) -> float:
        ...
        # ----------------------------------------------------------------
        # BUG: the percentage discount is applied here, BEFORE the quantity
        # discount below.  It should be applied AFTER quantity discounts so
        # that the percentage is taken off the already-reduced subtotal.
